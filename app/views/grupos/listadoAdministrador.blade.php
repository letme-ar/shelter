...
<div class="row" style="margin-bottom:20px;">
    <div class="col-md-6"><h2>Listados de grupos</h2>
    </div>
...
<table id="myTable" class="table">
    ...
    <tr style="background-color: #808080;">
        ...
        <td><strike>{{ $value->nombrecontacto }} </strike></td>
        <td><strike>{{ $value->telefonocontacto }}</strike></td>
        <td><strike>{{ $value->estilo }}</strike></td>
        <td><strike>{{ $value->negocio }}</strike></td>
        @endif
        <td>
            @if($value->eliminado != "1")
            <a href="{{ route('grupo.edit', $value->id) }}" class="btn btn-success btn-sm">
            Editar
            </a>
